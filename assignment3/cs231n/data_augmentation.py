...
def random_flips(X):
  ...
  for i in idx[:np.round(N/2)]:
    for c in xrange(C):
      out[i, c, :, :] = np.fliplr(out[i, c, :, :])
  #############################################################################
  #                           END OF YOUR CODE                                #
  #############################################################################
  return out
...
def random_crops(X, crop_shape):
  ...
  limx = H - HH
  limy = W - WW
  indx = np.random.randint(limx, size=N)
  indy = np.random.randint(limy, size=N)
  for n in xrange(N):
    out[n, :, :, :] = X[n, :, indx[n]:indx[n]+HH, indy[n]:indy[n]+WW]
  #############################################################################
  #                           END OF YOUR CODE                                #
  #############################################################################
  return out
...
def random_contrast(X, scale=(0.8, 1.2)):
  ...
  #############################################################################
  ctrs = [np.random.uniform(*scale) for i in xrange(N)]
  for i in xrange(N):
      out[i,:,:,:] *= ctrs[i]
  #############################################################################
  #                           END OF YOUR CODE                                #
...
def random_tint(X, scale=(-10, 10)):
  ...
  for n in xrange(N):
    for c in xrange(C):
      out[n, c, :, :] = X[n, c, :, :] + np.random.uniform(*scale)
  #############################################################################
  #                           END OF YOUR CODE                                #
  #############################################################################
